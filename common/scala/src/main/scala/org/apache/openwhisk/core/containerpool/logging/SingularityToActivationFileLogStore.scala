...
/**
 * Singularity based implementation of a LogStore.
 *
 * Relies on Singularity's implementation details with regards to the JSON log-driver. When using the JSON log-driver
 * Singularity writes stdout/stderr to a JSON formatted file which is read by this store. Logs are written in the
 ...
 */
class SingularityToActivationFileLogStore(system: ActorSystem, destinationDirectory: Path = Paths.get("logs"))
    extends SingularityToActivationLogStore(system) {
  ...
  override def collectLogs(transid: TransactionId,
    ...
    val toSeq = Flow[ByteString].via(SingularityToActivationLogStore.toFormattedString).toMat(Sink.seq[String])(Keep.right)
...
object SingularityToActivationFileLogStoreProvider extends LogStoreProvider {
  override def instance(actorSystem: ActorSystem): LogStore = new SingularityToActivationFileLogStore(actorSystem)
}
