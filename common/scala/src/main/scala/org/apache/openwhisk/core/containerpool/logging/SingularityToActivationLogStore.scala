...
/**
 * Represents a single log line as read from a Singularity log
 */
...
object SingularityToActivationLogStore {
 ...
 */
class SingularityToActivationLogStore(system: ActorSystem) extends LogStore {
  ...
  override def collectLogs(transid: TransactionId,
    ...
    logs
      .via(SingularityToActivationLogStore.toFormattedString)
      .runWith(Sink.seq)
      .flatMap { seq =>
...
object SingularityToActivationLogStoreProvider extends LogStoreProvider {
  override def instance(actorSystem: ActorSystem): LogStore = new SingularityToActivationLogStore(actorSystem)
}
